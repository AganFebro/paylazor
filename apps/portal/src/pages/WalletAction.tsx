...
import { getPostMessageTargetOrigin, getReferrerOrigin, isAllowedReferrerOrigin } from "../utils/postMessage"
...
export default function WalletAction() {
  ...
  useEffect(() => {
    ...
    if (!isAllowedReferrerOrigin(referrerOrigin)) {
      setStatus({
        message: `Blocked: untrusted opener/parent origin ${referrerOrigin ?? '(none)'}.`,
        type: 'error',
      })
      return
    }
  ...
  const handleError = (errorMessage: string, error?: any, isSignatureError = false) => {
    ...
    if (window.parent && window.parent !== window) {
      window.parent.postMessage(errorResponse, targetOrigin)
      return
    }
    ...
    // In popup window
    if (window.opener && window.opener !== window) {
      window.opener.postMessage(errorResponse, targetOrigin)
      if (isSignatureError) {
